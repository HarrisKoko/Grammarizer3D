    ...
    bool operator<(const HalfEdgeGraph& t) const {
        ...
        if (this->faceNormals[0] == t.faceNormals[0]) {
            if (this->faceNormals[1] == t.faceNormals[1]) {
                ...
                if (this->faceNormals[1].x == t.faceNormals[1].x) {
                    if (this->faceNormals[1].y == t.faceNormals[1].y) {
                        return (this->faceNormals[1].z < t.faceNormals[1].z);
                    }
                    return (this->faceNormals[1].y < t.faceNormals[1].y);
                }
                return (this->faceNormals[1].x < t.faceNormals[1].x);

            }
        } else {
            if (this->faceNormals[0].x == t.faceNormals[0].x) {
                if (this->faceNormals[0].y == t.faceNormals[0].y) {
                    return (this->faceNormals[0].z < t.faceNormals[0].z);
                }
                return (this->faceNormals[0].y < t.faceNormals[0].y);
            }
            return (this->faceNormals[0].x < t.faceNormals[0].x);
        }
#else
        if (this->angle == t.angle) {
            if (this->faces[0] == t.faces[0]) {
